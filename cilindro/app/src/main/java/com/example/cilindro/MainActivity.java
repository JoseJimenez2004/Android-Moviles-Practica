...
class Perspectiva extends View {
    ...
    private void drawCylinder(Canvas canvas) {
        ...
        for (int i = 0; i < segments; i++) {
            drawLine(canvas, i, (i + 1) % segments);
        }

        // Dibujar la base superior
        for (int i = segments; i < 2 * segments; i++) {
            drawLine(canvas, i, segments + (i + 1) % segments);
        }

        // Dibujar las aristas verticales
        for (int i = 0; i < segments; i++) {
            drawLine(canvas, i, i + segments);
        }
    }
...
class Obj {
    ...
    int segments = 20; // Número de segmentos para aproximar el círculo
    Point3D[] w;
    Point2D[] vScr;

    Obj() {
        w = new Point3D[segments * 2];
        vScr = new Point2D[segments * 2];

        // Crear puntos para la base inferior (z = -1)
        for (int i = 0; i < segments; i++) {
            double angle = 2 * Math.PI / segments * i;
            w[i] = new Point3D(Math.cos(angle), Math.sin(angle), -1);
        }

        // Crear puntos para la base superior (z = 1)
        for (int i = 0; i < segments; i++) {
            double angle = 2 * Math.PI / segments * i;
            w[i + segments] = new Point3D(Math.cos(angle), Math.sin(angle), 1);
        }

        objSize = 2; // Tamaño del cilindro
        rho = 5 * objSize;
    }
